...
const Party = ({ diner, currentGuest, guests, bill, selectGuest, showError }) => {
    ...
    const guestTemplate = (limit) =>
        new Array(limit).fill('_').map((_, index) => (
            <button
                key={index}
                className={`guest ${isSelected(index) ? 'selected' : 'unselected'}`}
                style={{
                    backgroundColor: ` ${isSelected(index) ? COLOURS[index] : '#FFFFFF'}`,
                    borderColor: ` ${isSelected(index) ? '#000000' : COLOURS[index]}`,
                    color: ` ${isSelected(index) ? '#000000' : COLOURS[index]}`,
                }}
                onClick={() => {
                    selectGuest(index + 1);
                }}
    ...
    return (
        <section className="party">
            ...
            <div className={`party__bill ${bill ? 'show' : ''}`}>
